...
CClient::CClient (int port, char *hostname, bool bObserv)
{
  ...
  if (memcmp(pmyNet->GetQueue(),n_servconack,
	     strlen(n_servconack)) != 0) {
...
{
  delete pmyNet;
  if (pmyWorld!=NULL) delete pmyWorld;
  if (aTms!=NULL) {
    for (UINT i=0; i<numTeams; i++)
      if (aTms[i]!=NULL) delete aTms[i]; 
  }
  delete [] aTms;
}
...
UINT CClient::ReceiveWorld()
{
  ...
  while (pmyNet->GetQueueLength()<sizeof(UINT)) {
    pmyNet->CatchPkt();
    if (IsOpen()==0) return 0;  // Eek!  World disappeared!
  }

  memcpy (&netlen, buf, sizeof(UINT));
  len = ntohl(netlen);

  if (len>MAX_THINGS*256) return 0;  // Bad!
...
void CClient::MeetTeams()
{
  ...
  for (nTm=0; nTm<numTeams; nTm++) {
    while ((len=pmyNet->GetQueueLength())
	   < aTms[nTm]->GetSerInitSize()) {
      pmyNet->CatchPkt();
    }
...
int CClient::SendAck ()
{
  if (IsOpen()==0) return 0;    // Don't write to closed conn
...
void CClient::DoTurn()
{
  ...
  if (IsOpen()==0) return;  // Don't write to closed conn

  CTeam *pTm = aTms[umyIndex];
  UINT len = pTm->GetSerialSize();
  if (len>4096) return;  // BAD!
