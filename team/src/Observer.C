...
Observer::Observer(char *regFileName, int gfxFlag) {
  ...
  // Create the window
  win = XCreateSimpleWindow(display, RootWindow(display, screenNum), 0, 0,
                            displayWidth, displayHeight, 0, black, gray);

  gc = XCreateGC(display, win, 0, NULL);
  //  XSetLineAttributes(display,gc, 3,LineSolid,CapRound,JoinBevel);

  // Set some properties of the window
  XSetStandardProperties(display, win, "MechMania IV: The Vinyl Frontier",
  ...
  canvas = XCreatePixmap(display, win, displayWidth, displayHeight,
  ...
  tHeight = (int)((font_info->ascent + font_info->descent) * 7.3);
  ...
  spaceCanvas = XCreatePixmap(display, win, spaceWidth, spaceHeight,
                              DefaultDepth(display, screenNum));
  ...
  // load the logo
#ifdef USE_XPMLIB
  XpmReadFileToPixmap(display, win, "gfx/MM4Logo.xpm", &logoPix, &logoClip,
                      &xa);
...
int Observer::plotWorld() {
  ...
  for (UINT i = myWorld->UFirstIndex; i != (UINT)-1;
    ...
    if (aThing->GetKind() != ASTEROID) {
      if (drawnames == 1) {
        snprintf(thname, sizeof(thname), "%s", aThing->GetName());
      } else if (drawnames == 2) {
    ...
    if (getUseXpm() == true && myViewer->gotImages == true) {
      if (aThing->GetKind() == SHIP) {
        ...
        myViewer->plotThing(pX, pY, ang, type, NULL);
      }
      ...
      XDrawString(display, spaceCanvas, gc, txtX, txtY, thname, strlen(thname));
      XSetFont(display, gc, font_info->fid);
      ...
      if (aThing->GetKind() == ASTEROID) {
        ...
        if (astPtr->GetMaterial() == URANIUM) {
          ...
          XSetForeground(display, gc, vinylcol);
        }
        ...
        // Scale and center
        XDrawArc(display, spaceCanvas, gc, (pX - rad), (pY - rad), (rad * 2),
                 (rad * 2), 0, 23040);
        ...
        if (aThing->GetKind() == STATION) {
          ...
          XDrawRectangle(display, spaceCanvas, gc, pX, pY, endX, endY);
        } else {
          // Thing is a ship, calculate triangle points
          triPts[0].x = pX + (0.7071 * (rad * cos(ang)));
          triPts[0].y = pY + (0.7071 * (rad * sin(ang)));
          triPts[1].x = pX + (0.7071 * (rad * cos(ang + oneThirdCircle)));
          triPts[1].y = pY + (0.7071 * (rad * sin(ang + oneThirdCircle)));
          triPts[2].x = pX + (0.7071 * (rad * cos(ang + twoThirdCircle)));
          triPts[2].y = pY + (0.7071 * (rad * sin(ang + twoThirdCircle)));
          ...
          XDrawLine(display, spaceCanvas, gc, triPts[0].x, triPts[0].y,
...
UINT Observer::AlertStatus(double statamt, double statcap) {
  ...
  if (statamt > 0.5 * statcap) {
    return okCol;
  }
  if (statamt > 0.2 * statcap) {
    return warnCol;
  }
  return critCol;
}
...
void Observer::plotStatusWins(int Teamnum, Pixmap tCanvas) {
  ...
  snprintf(bldstr, sizeof(bldstr), "%2.2d: %-45.45s", pTeam->GetTeamNumber(),
           pTeam->GetName());
  ypos += font_height;
  XDrawString(display, tCanvas, gc, 5, ypos, bldstr, strlen(bldstr));
  ...
  XSetForeground(display, gc, tmcol);
  snprintf(bldstr, sizeof(bldstr), "%14s: %.3f", pTeam->GetStation()->GetName(),
           pTeam->GetScore());
  XDrawString(display, tCanvas, gc, 5 + font_width * 15, ypos, bldstr,
              strlen(bldstr));
  ...
  ypos += font_height;
  XSetForeground(display, gc, gray);
  XDrawString(display, tCanvas, gc, 5, ypos, title, strlen(title));
  ...
  for (UINT shnum = 0; shnum < pTeam->GetShipCount(); shnum++) {
    ...
    XDrawString(display, tCanvas, gc, xpos, ypos, bldstr, strlen(bldstr));
    ...
    if (pSh->IsDocked()) {
      snprintf(bldstr, sizeof(bldstr), " Docked");
    }
    XDrawString(display, tCanvas, gc, xpos, ypos, bldstr, strlen(bldstr));
...
void Observer::scrollUp(void) {
  ...
  XCopyArea(display, msgCanvas, msgCanvas, gc, 0, font_height, msgWidth,
            font_height * (msg_rows - 1), 0, 0);
  XSetForeground(display, gc, black);
  XFillRectangle(display, msgCanvas, gc, 0, font_height * (msg_rows - 1),
                 msgWidth, msgHeight);
}
...
void Observer::printMsg(char *str, int color) {
  ...
  while (*str) {
    ...
    if (*str == '\n') {
      ...
      XSetForeground(display, gc, color);
      XDrawString(display, msgCanvas, gc, (msg_c * font_width) + 5,
                  (msg_r * font_height) + font_info->ascent, tmpstr, 1);
      msg_c++;
    }
...
void Observer::printGameTime(double game_time) {
  ...
  XSetForeground(display, gc, white);
  XDrawString(display, timeCanvas, gc, 0, (int)(font_info->ascent * 1.5), str,
              strlen(str));
}
...
void Observer::drawAll() {
  ...
  XCopyArea(display, timeCanvas, win, gc, 0, 0, timeWidth, timeHeight, timeX,
            timeY);
  XCopyArea(display, t1Canvas, win, gc, 0, 0, tWidth, tHeight, t1PosX, t1PosY);
  XCopyArea(display, msgCanvas, win, gc, 0, 0, msgWidth, msgHeight, msgPosX,
            msgPosY);
  XCopyArea(display, t2Canvas, win, gc, 0, 0, tWidth, tHeight, t2PosX, t2PosY);

  if (attractor) {
    XSetClipMask(display, gc, logoClip);
    XSetClipOrigin(display, gc, (spaceWidth - logoW) / 2,
                   (spaceHeight - logoH) / 2);
...
void Observer::drawCredits() {
  ...
  char cr1[] =
      ...
      "         'G'";
  char cr2[] =
      "The Vinyl Frontier               Stars           Names          "
      "Velocities       Graphics";
...
void Observer::getKeystroke() {
  ...
  if (result == 0) {
    return;  // No events to pick up.
  }

  if (event.type != KeyPress) {
    return;
  }
  if (XLookupString(&event.xkey, text, 31, &key, 0) != 1) {
    return;
  }
...
void Observer::setAttractor(int val) { attractor = val; }

bool Observer::getUseXpm() { return useXpm; }
...
bool Observer::getUseVelVectors() { return useVelVectors; }
