...
bool ArgumentParser::Parse(int argc, char* argv[]) {
    try {
        cxxopts::Options options("mm4serv/mm4obs", "MechMania IV: The Vinyl Frontier");

        options.add_options()
            ("p,port", "Server port", cxxopts::value<int>()->default_value("2323"))
            ("h,hostname", "Server hostname", cxxopts::value<std::string>()->default_value("localhost"))
            ("g,gfxreg", "Graphics registry file", cxxopts::value<std::string>()->default_value("graphics.reg"))
            ("T,numteams", "Number of teams", cxxopts::value<int>()->default_value("2"))
            ("G,graphics", "Enable full graphics mode")
        ...
        // Feature flags
        options.add_options("Features")
            ("old-collision-detection", "Use old collision detection")
        ...
        // Feature bundles
        options.add_options("Bundles")
            ("improved-physics", "Enable all new physics features")
        ...
        if (result.count("old-collision-detection")) features["collision-detection"] = false;
        if (result.count("new-collision-detection")) features["collision-detection"] = true;
        if (result.count("old-elastic-collisions")) features["elastic-collisions"] = false;
        if (result.count("new-elastic-collisions")) features["elastic-collisions"] = true;
        if (result.count("old-conserve-momentum")) features["conserve-momentum"] = false;
        if (result.count("new-conserve-momentum")) features["conserve-momentum"] = true;
...
bool ArgumentParser::ParseConfigJson(const std::string& content) {
    ...
    // Format: { "profile": "name", "features": { "feature-name": "new/old", ... }, "options": { ... } }

    // This is a simplified parser - in production you'd want to use a proper JSON library
    ...
    while (std::getline(stream, line)) {
        ...
        if (line.find("\"features\"") != std::string::npos) {
            ...
            else if (key == "gfxreg") gfxreg = value;
            else if (key == "numteams") numteams = std::stoi(value);
            else if (key == "graphics") gfxflag = (value == "true");
            else if (key == "reconnect") reconnect = retry = (value == "true");
        }
