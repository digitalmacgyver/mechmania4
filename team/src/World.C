  ...
  for (nteam = 0; nteam < GetNumTeams(); ++nteam) {
    ...
    for (nship = 0; nship < pTeam->GetShipCount(); ++nship) {
      ...
      if (pTarget != NULL) {
        ...
        // Record impact angle for rendering overlays (skip if ship is docked)
        bool target_is_docked_ship = false;
        if (pTarget->GetKind() == SHIP) {
          CShip* target_ship = static_cast<CShip*>(pTarget);
          target_is_docked_ship = target_ship->IsDocked();
        }
        if (!target_is_docked_ship) {
          double impact_angle = pTarget->GetPos().AngleTo(LasThing.GetPos());
          pTarget->bIsGettingShot = impact_angle;
        }
...
unsigned int CWorld::CollisionEvaluationNew() {
  ...
  // Record impact directions for rendering overlays (ships, stations, etc.).
  // Legacy collision handling set bIsColliding / bIsGettingShot when a collision
  // occurred; replicate that behaviour here so observers can show shield flashes.
  auto record_hit_direction = [](CThing* self, CThing* other) {
    if (!self || !other) {
      return;
    }
    if (other->GetKind() == SHIP) {
      CShip* other_ship = static_cast<CShip*>(other);
      if (other_ship->IsDocked()) {
        return;  // Ignore collisions generated by docked ships
      }
    }
    if (self->GetKind() == SHIP) {
      CShip* ship = static_cast<CShip*>(self);
      if (ship->IsDocked()) {
        return;  // Docked ships never show hit overlays
      }
    }
    double angle = self->GetPos().AngleTo(other->GetPos());
    if (other->GetKind() == GENTHING) {
      self->bIsGettingShot = angle;
    } else {
      self->bIsColliding = angle;
    }
  };
  for (const CollisionPair& pair : collisions) {
    record_hit_direction(pair.object1, pair.object2);
    record_hit_direction(pair.object2, pair.object1);
  }
