...
{
  if (conn<1 || conn>maxconn) return -1;
  return queuelen[conn-1];
}

char *CNetwork::GetQueue (int conn)
{
  if (conn<1 || conn>maxconn) return NULL;
  return queue[conn-1];
}

void CNetwork::FlushQueue (int conn)
{
  if (conn<1 || conn>maxconn) return;
  queuelen[conn-1]=0;
}
