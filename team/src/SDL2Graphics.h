...
class SDL2Graphics {
 ...
 public:
  ...
  void DrawCircle(int cx, int cy, int radius, const Color& color,
                  bool filled = false);
  void DrawArc(int cx, int cy, int radius, double startAngle, double endAngle,
               const Color& color);
  ...
  void DrawText(const std::string& text, int x, int y, const Color& color,
                bool small = false, bool bold = true);
  ...
  void GetTextSizeEx(const std::string& text, int& w, int& h,
                     bool small = false, bool bold = false);

  // Image handling
  SDL_Texture* LoadImage(const std::string& path);
  SDL_Texture* LoadXPM(const char** xpmData);
  void DrawImage(SDL_Texture* image, int x, int y, double angle = 0.0,
                 double scale = 1.0);
