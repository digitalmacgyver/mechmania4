...
namespace {
using groogo::laser::FutureLineOfFire;
using groogo::laser::LaserResources;
using groogo::laser::NormalizeAngle;
using groogo::laser::EvaluateFiringPredictability;
...
void GetVinyl::Decide() {
  // Strategic planning has already been done in Groogo::AssignShipOrders()
  ...
  Groogo* groogo_team = static_cast<Groogo*>(pmyTeam);
  bool no_vinyl_free = (fabs(groogo_team->vinyl_left) <= g_fp_error_epsilon);
  bool no_resources_free = (fabs(groogo_team->uranium_left) <= g_fp_error_epsilon &&
                            fabs(groogo_team->vinyl_left) <= g_fp_error_epsilon);
  ...
  double fuel_reserve = groogo::constants::FUEL_RESERVE;
  ...
  if (pShip->GetOrder(O_LASER) == 0.0) {
    LaserResources laser =
        groogo::laser::ComputeLaserResources(pShip, fuel_reserve);
...
EmergencyOrders GetVinyl::HandleImminentCollision(std::vector<CThing *> collisions, unsigned int turns, EmergencyOrders emergency_orders) {
  ...
  for (CThing *athing : collisions) {
    ...
    double fuel_allowed =
        pShip->GetAmount(S_FUEL) - groogo::constants::FUEL_RESERVE;
    if (fuel_allowed < 0.0) {
      fuel_allowed = 0.0;
    }
    ...
    // Check resource availability for shield strategy
    Groogo* groogo_team = static_cast<Groogo*>(pmyTeam);
    bool world_has_vinyl = (fabs(groogo_team->vinyl_left) > g_fp_error_epsilon);
    bool world_has_uranium = (fabs(groogo_team->uranium_left) > g_fp_error_epsilon);
