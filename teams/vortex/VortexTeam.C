...
void VortexCollector::HandleEmergency() {
    AvoidCollisions();
}
...
bool VortexCollector::ShouldReturnHome() {
    ...
    if (pShip->GetAmount(S_CARGO) > 21.0) return true;

    // Return if fuel is getting low
    if (pShip->GetAmount(S_FUEL) < 10.0) return true;

    // Return if no vinyl left and have cargo
    VortexTeam* team = (VortexTeam*)pShip->GetTeam();
    if (team->total_vinyl < 1.0 && pShip->GetAmount(S_CARGO) > 1.0) return true;
    ...
    if (pShip->GetAmount(S_CARGO) > 15.0) {
        ...
        if (dist_home < 200.0) return true;
    }
...
bool VortexCollector::IsInMyZone(CThing* thing) {
    ...
    switch (assigned_zone) {
        case 0: return (pos.fX < 0 && pos.fY < 0);  // Bottom-left
        case 1: return (pos.fX >= 0 && pos.fY < 0);  // Bottom-right
        case 2: return (pos.fX < 0 && pos.fY >= 0);  // Top-left
        case 3: return (pos.fX >= 0 && pos.fY >= 0); // Top-right
    }
    return false;
}
...
void VortexCollector::AvoidCollisions() {
    ...
    for (UINT i = world->UFirstIndex; i != BAD_INDEX; i = world->GetNextIndex(i)) {
        CThing* thing = world->GetThing(i);
        if (!thing || thing == pShip || thing == current_target) continue;
        ...
        if (kind == SHIP || (kind == ASTEROID &&
            ...
            if (evade_angle > PI) evade_angle -= PI2;
            if (evade_angle < -PI) evade_angle += PI2;
...
void VortexCollector::ConsiderCombat() {
    ...
    if (pShip->GetAmount(S_FUEL) < 15.0) return;
