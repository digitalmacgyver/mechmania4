#!/bin/bash
# Test 3: Ship-Ship Collision
# Expected: Two ships from same team collide with each other
...
echo "Running Test 3: Ship-Ship Collision (20 turns)..."
echo "Command: python3 scripts/test_collision_modes.py --team1 noop --team2 testteam --test-file teams/testteam/tests/test3_ship_ship_collision.txt --max-turns 20"
echo ""
...
echo "=========================================="
echo "Test 3: Ship-Ship Collision"
echo "=========================================="
...
if [ -f "$LEGACY_LOG" ]; then
    # Look for Test-1 and Test-2 colliding (symmetric - either order)
    COLLISION_COUNT=$(grep "COLLISION_DETECTED" "$LEGACY_LOG" | grep -E "Test-1.*Test-2|Test-2.*Test-1" | wc -l)
    if [ "$COLLISION_COUNT" -gt 0 ]; then
        echo "  ✓ Ship-ship collision detected: $COLLISION_COUNT collisions"
        LEGACY_PASS=1
    else
        echo "  ✗ No ship-ship collision detected"
        LEGACY_PASS=0
    fi
else
...
if [ -f "$NEW_LOG" ]; then
    # Look for Test-1 and Test-2 colliding (symmetric - either order)
    COLLISION_COUNT=$(grep "COLLISION_DETECTED" "$NEW_LOG" | grep -E "Test-1.*Test-2|Test-2.*Test-1" | wc -l)
    if [ "$COLLISION_COUNT" -gt 0 ]; then
        echo "  ✓ Ship-ship collision detected: $COLLISION_COUNT collisions"
        NEW_PASS=1
    else
